    ...
    <div className="min-h-screen bg-[#001010] text-cyan-50 pt-24 pb-12 px-4 md:pt-32 md:pb-16 md:px-8 relative overflow-hidden selection:bg-cyan-500/30 font-mono">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-cyan-600/80 mb-8 border-b border-cyan-900/50 pb-4">
          <Link href="/kd-method" className="hover:text-cyan-400 transition-colors whitespace-nowrap drop-shadow-[0_0_5px_rgba(34,211,238,0.5)]">KD Method</Link>
          <span className="text-cyan-800">&gt;</span>
          <Link href={`/kd-method/${subjectSlug}`} className="hover:text-cyan-400 transition-colors whitespace-nowrap drop-shadow-[0_0_5px_rgba(34,211,238,0.5)]">{subjectTitle}</Link>
          {path.map((segment, index) => {
            ...
            const href = `/kd-method/${subjectSlug}/${path.slice(0, index + 1).join('/')}`;
            return (
              <span key={index} className="flex items-center gap-x-2">
                <span className="text-cyan-800">&gt;</span>
        ...
        {node.children && node.children.length > 0 && (
          ...
          >
            {node.children.map((child: any, i: number) => {
              const href = `/kd-method/${subjectSlug}/${path.join('/')}/${child.slug}`;
              return (
